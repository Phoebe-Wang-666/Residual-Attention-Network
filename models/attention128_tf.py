...
# Residual Attention Network — Attention-128
class ResidualAttentionModel128(keras.Model):
    """
    Architecture:
    ...
    def __init__(self, num_classes=1000, att_type="arl", **kwargs):
        ...
        # STEM: He/Kaiming initialization for all Conv2D layers (as per paper)
        he_init = HeNormal()
        ...
        # STAGE 1
        # Pre: 1 bottleneck block (64 -> 256)
        self.pre1, in_ch = make_preact_layer(block, in_ch, 64, blocks=1)
        
        # Attention: 3 trunk blocks + mask depth 2
        #self.att1 = AttentionModule(block, in_ch, trunk_blocks=3, mask_depth=2, att_type=att_type)
        self.att1_1=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=2,att_type=att_type)
        self.att1_2=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=2,att_type=att_type)
        self.att1_3=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=2,att_type=att_type)
        ...
        # Attention: 6 trunk blocks + mask depth 1
        self.att2_1=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=1,att_type=att_type)
        self.att2_2=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=1,att_type=att_type)
        self.att2_3=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=1,att_type=att_type)
        ...
        # Attention: 3 trunk blocks + mask depth 1
        #self.att3 = AttentionModule(block, in_ch, trunk_blocks=3, mask_depth=1, att_type=att_type)
        self.att3_1=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=1,att_type=att_type)
        self.att3_2=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=1,att_type=att_type)
        self.att3_3=AttentionModule(block,in_ch,trunk_blocks=2,mask_depth=1,att_type=att_type)
        # Post: 1 bottleneck block
    ...
    def call(self, inputs, training=None):
        ...
        x=self.att1_1(x,training=training)
        x=self.att1_2(x,training=training)
        x=self.att1_3(x,training=training)
        ...
        x=self.att2_1(x,training=training)
        x=self.att2_2(x,training=training)
        x=self.att2_3(x,training=training)
        x=self.post2(x,training=training)


        # Stage 3
        x = self.pre3(x, training=training)
        x=self.att3_1(x,training=training)
        x=self.att3_2(x,training=training)
        x=self.att3_3(x,training=training)
...
if __name__ == "__main__":
    model = ResidualAttentionModel128(num_classes=1000)
